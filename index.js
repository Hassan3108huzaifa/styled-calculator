...
import ora from "ora";
async function typeWelcomeMessage(message) {
...
async function animateAddition(num1, num2) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    return num1 + num2;
}
async function animateSubtraction(num1, num2) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    return num1 - num2;
}
async function animateMultiplication(num1, num2) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    return num1 * num2;
}
async function animateDivision(num1, num2) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    if (num2 === 0) {
...
async function animateExponentiation(base, exponent) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    return Math.pow(base, exponent);
}
async function animateSquareRoot(number) {
    const spinner = ora(chalk.bold.yellow("Calculating...")).start();
    await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
    spinner.stop();
    if (number < 0) {
